...
class IterationProfiler:
    def __init__(
        self, iteration, input_provider, path_to_entry_point_dir, project_root
    ):
    ...
    def measure_run_time_ms(self, batch_size, initial_repetitions=None):
        ...
        while repetitions <= max_repetitions:
            ...
            logger.debug(
                "Iters: %d, Measured: %f (range: %f)",
                doubled,
                greater,
                max(lesser, greater) / min(lesser, greater),
            )
    ...
    def sample_run_time_ms_by_batch_size(
        ...
        if (
            start_batch_size_run_time_ms is None
            or start_batch_size_peak_usage_bytes is None
        ):
            start_run_time_ms, start_peak_usage_bytes, _ = self.measure_run_time_ms(
                start_batch_size
            )
        else:
        ...
        samples.append(
            IterationSample(start_batch_size, start_run_time_ms, start_peak_usage_bytes)
        )
    ...
    def _sample_range(self, min_size, max_size, num_samples, is_increasing=True):
        ...
        while len(samples) < num_samples and len(stack) > 0:
            ...
            logger.debug("[%d, %d] Sampling batch size: %d", lower, upper, next_size)
