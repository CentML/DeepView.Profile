...
class UtilizationProfiler:
    ...
    def _calculate_gpu_times(self, kernel_list):
        ...
        start, end, netTime = (
            kernel_list[0][0],
            kernel_list[0][1],
            kernel_list[0][1] - kernel_list[0][0],
        )
    ...
    def _calculate_backward_cpu_span(self, node):
        ...
        for bw_slice in backward_slices:
            ...
            kernel_slices.extend(
                [
                    (kernel["ts"], kernel["ts"] + kernel["dur"])
        ...
        for i in range(N):
            for j in range(M):
                ...
                if (
                    i > 0
                    and j > 0
                    and self._can_match(forward[i].name, backward[j]["name"])
                ):
    ...
    def _filter_traces(self, raw_slices):
        ...
        filtered_slices = [
            event
            for idx, event in enumerate(raw_slices["traceEvents"])
            if idx not in idx_to_filter
        ]
...
def utilization_analysis(
    ...
    jsonFormat = {
        "root_node": utilization._convert_node_to_dict(utilization._root_node),
        "tensor_core_perc": utilization._tensor_core_perc,
    }
    return jsonFormat
